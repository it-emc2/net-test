...
class ApiService {
  ...
  async searchCustomers(query) {
    return this.request("GET", "/api/customers/search", {
      params: { q: query },
    });
  }
  ...
  async saveDraft(name, offerType, payload) {
    return this.request("POST", "/api/drafts", {
      body: { name, offerType, payload },
    });
  }

  async searchDrafts(offerType, query) {
    return this.request("GET", "/api/drafts/search", {
      params: { offerType, q: query },
    });
  }
  ...
  async loadOffer(offerNumber) {
    return this.request(
      "GET",
      `/api/offers/${encodeURIComponent(offerNumber)}`,
    );
  }
