...
(function initSmartTraySearch() {
  ...
  function saveTraySelection(value, productId) {
    try {
      localStorage.setItem(TRAY_KEY, JSON.stringify({ value, productId }));
    } catch {}
  }
  function loadTraySelection() {
    try {
      return JSON.parse(localStorage.getItem(TRAY_KEY) || "{}");
    } catch {
      return {};
    }
  }
  ...
  function renderRadioSuggestions(list) {
    ...
    const alreadyChecked = !!form?.querySelector(
      'input[name="traySize"]:checked'
    );

    const radiosHtml = list
      .slice(0, 3)
      .map((p, idx) => {
    ...
    out.querySelectorAll('input[name="traySize"]').forEach((radio) => {
      radio.addEventListener("change", () => {
        ...
        if (listEl) {
          listEl
            .querySelectorAll("label.radio-pill")
    ...
    if (saved?.value) {
      ...
      const match = out.querySelector(
        `#traySuggestionRadioList input[name="traySize"][value="${esc(
          saved.value
        )}"]`
      );
      if (match) {
  ...
  function maybeAutoFetch() {
    const hasValues = [wEl, lEl, hEl].every(
