// Ported from legacy src/routes/routing.js — POST /suggest-distance.
// Resolves the company origin (env coords or geocoded COMPANY_ADDRESS),
// geocodes the customer address via a Photon -> ORS -> Nominatim cascade
// (with address-variant fallbacks), then computes road distance/duration via
// an ORS -> OSRM cascade. Behavior mirrors the legacy handler.
...
interface SuggestDistanceResponse {
  ok: true;
  oneWayKm: number;
  roundTripKm: number;
  oneWaySeconds: number;
  roundTripSeconds: number;
  from: { address: string; geocoded: string; lat: number; lng: number };
  to: {
    address: string;
    geocoded: string;
    usedVariant?: string;
 ...
 */
function buildAddressVariants({ street, postalCode, city, state, country }: AddressParts): string[] {
 ...
 */
function buildCustomerAddress({ street, postalCode, city, state, country }: AddressParts): string {
  const parts = [
    street || "",
...
function geocodeResultMatchesInput(result: GeoResult | null, addressParts: AddressParts = {}): boolean {
 ...
 */
async function geocodeWithPhoton(address: string): Promise<GeoResult> {
 ...
 */
async function geocodeWithORS(address: string): Promise<GeoResult> {
  if (!ORS_API_KEY) {
 ...
 */
async function geocodeSingleAddress(address: string): Promise<GeoResult> {
  const errors: string[] = [];
 ...
 */
async function geocodeWithVariants(addressParts: AddressParts): Promise<GeoResult> {
 ...
 */
async function getCompanyCoords(): Promise<GeoResult> {
...
async function osrmDistanceAndDuration(a: Coords, b: Coords): Promise<RouteResult> {
...
async function orsDistanceAndDuration(a: Coords, b: Coords): Promise<RouteResult> {
...
async function getRoadDistanceAndDuration(a: Coords, b: Coords): Promise<RouteResult> {
  const errors: string[] = [];
 ...
 */
router.post("/suggest-distance", async (req: Request, res: Response) => {
  try {
    const k: any = req.body?.Kundendaten || req.body?.bereich || req.body || {};
    ...
    const payload: SuggestDistanceResponse = {
      ok: true,
      oneWayKm,
      roundTripKm,
      oneWaySeconds,
